...
public class HelloConsumerMicroservice extends AbstractVerticle {
  ...
  private void invokeMyFirstMicroservice(RoutingContext rc) {
    HttpRequest<JsonObject> request1 =
        client.get(8080, "localhost", "/Matias").as(BodyCodec.jsonObject());

    HttpRequest<JsonObject> request2 =
        client.get(8080, "localhost", "/Miquito").as(BodyCodec.jsonObject());
    ...
    Single
      .zip(s1, s2, (luke, leia) -> {
        // We have the results of both requests in Matias and Miquito
        return new JsonObject()
            .put("Matias", luke.getString("message"))
            .put("Miquito", leia.getString("message"));
       })
      .subscribe(
